...
int MosMain() {
	...
	MdigProcess(MilDigitizer[0], MilImageBuf[0], 15, M_START, M_TRIGGER_FOR_FIRST_GRAB, ProcessingFunction, &UserHookData[0]);
	MdigProcess(MilDigitizer[1], MilImageBuf[1], 15, M_START, M_TRIGGER_FOR_FIRST_GRAB, ProcessingFunction, &UserHookData[1]);
	MdigProcess(MilDigitizer[2], MilImageBuf[2], 15, M_START, M_TRIGGER_FOR_FIRST_GRAB, ProcessingFunction, &UserHookData[2]);
	MdigProcess(MilDigitizer[3], MilImageBuf[3], 15, M_START, M_TRIGGER_FOR_FIRST_GRAB, ProcessingFunction, &UserHookData[3]);
...
MIL_INT MFTYPE ProcessingFunction(MIL_INT HookType, MIL_ID HookId, void* HookDataPtr)
{
	...
	Mat cvImage;
	cvImage.create(Size(UserHookDataPtr->Width, UserHookDataPtr->Height), CV_8U);
	MbufGet(SaveBufferId, cvImage.data);
