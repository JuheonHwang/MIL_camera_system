...
int MosMain() {
	...
	MIL_ID own_id[4];
	MsysInquire(MilSystem[0], M_OWNER_APPLICATION, &own_id[0]);
	MsysInquire(MilSystem[1], M_OWNER_APPLICATION, &own_id[1]);
	MsysInquire(MilSystem[2], M_OWNER_APPLICATION, &own_id[2]);
	MsysInquire(MilSystem[3], M_OWNER_APPLICATION, &own_id[3]);

	if (own_id[0] == MilApplication)
	{
		MosPrintf(MIL_TEXT("0 is same with MilApplication.\n"));
	}

	MIL_STRING_STREAM QueueStream[2];
	QueueStream[0] << MIL_TEXT("C:/output_image/") << 1 << MIL_TEXT("_") << 3 << MIL_TEXT(".tiff");
	QueueStream[1] << MIL_TEXT("C:/output_image/") << 1 << MIL_TEXT("_") << 4 << MIL_TEXT(".tiff");
	MappFileOperation(own_id[2], QueueStream[0].str(), MilApplication, QueueStream[0].str(), M_FILE_MOVE, M_DEFAULT, M_NULL);
	MappFileOperation(own_id[3], QueueStream[1].str(), MilApplication, QueueStream[1].str(), M_FILE_MOVE, M_DEFAULT, M_NULL);
