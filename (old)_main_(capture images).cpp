...
int MosMain() {
	...
	if (fs::is_empty(output_folder))
	{
		imwrite(output_folder + "/" + "1_1.png", cvImage[0]);
		imwrite(output_folder + "/" + "1_2.png", cvImage[1]);
		imwrite(output_folder + "/" + "1_3.png", cvImage[2]);
		imwrite(output_folder + "/" + "1_4.png", cvImage[3]);
	}
	else
	{
		...
		int idx = lastFileName_[0] - '0';

		string idx_ = to_string(idx + 1);
