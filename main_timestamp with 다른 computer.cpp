...
int MosMain() {
	...
	CHK(MsysAlloc(M_SYSTEM_RAPIXOCXP, M_DEV0, M_DEFAULT, &MilSystem[0]));
	MsysControl(MilSystem[0], M_MODIFIED_BUFFER_HOOK_MODE, M_MULTI_THREAD);
	CHK(MdispAlloc(MilSystem[0], M_DEFAULT, MIL_TEXT("M_DEFAULT"), M_WINDOWED, &MilDisplay[0]));
	CHK(MdigAlloc(MilSystem[0], M_DEV0, MIL_TEXT("M_DEFAULT"), M_DEFAULT, &MilDigitizer[0]));

	CHK(MsysAlloc(M_SYSTEM_RAPIXOCXP, M_DEV1, M_DEFAULT, &MilSystem[1]));
	MsysControl(MilSystem[1], M_MODIFIED_BUFFER_HOOK_MODE, M_MULTI_THREAD);
	CHK(MdispAlloc(MilSystem[1], M_DEFAULT, MIL_TEXT("M_DEFAULT"), M_WINDOWED, &MilDisplay[1]));
	CHK(MdigAlloc(MilSystem[1], M_DEV0, MIL_TEXT("M_DEFAULT"), M_DEFAULT, &MilDigitizer[1]));

	CHK(MsysAlloc(MIL_TEXT("dmiltcp://***.***.***.***:57010/M_SYSTEM_RAPIXOCXP"), M_DEV0, M_DEFAULT, &MilSystem[2]));
	MsysControl(MilSystem[2], M_MODIFIED_BUFFER_HOOK_MODE, M_MULTI_THREAD);
	CHK(MdispAlloc(MilSystem[2], M_DEFAULT, MIL_TEXT("M_DEFAULT"), M_WINDOWED, &MilDisplay[2]));
	CHK(MdigAlloc(MilSystem[2], M_DEV0, MIL_TEXT("M_DEFAULT"), M_DEFAULT, &MilDigitizer[2]));

	CHK(MsysAlloc(MIL_TEXT("dmiltcp://***.***.***.***:57010/M_SYSTEM_RAPIXOCXP"), M_DEV1, M_DEFAULT, &MilSystem[3]));
	MsysControl(MilSystem[3], M_MODIFIED_BUFFER_HOOK_MODE, M_MULTI_THREAD);
	CHK(MdispAlloc(MilSystem[3], M_DEFAULT, MIL_TEXT("M_DEFAULT"), M_WINDOWED, &MilDisplay[3]));
	CHK(MdigAlloc(MilSystem[3], M_DEV0, MIL_TEXT("M_DEFAULT"), M_DEFAULT, &MilDigitizer[3]));
...
void saveQueue(atomic <bool>& stop)
{
	while (true)
	{
		if (!save_queue.empty())
		{
			...
			{
				MbufSave("remote:///C:/output_image/" + s_idx.str() + "_" + d_idx.str() + "_" + to_string(front_queue.timestamp) + ".tiff", front_queue.GrabImage);

				MbufFree(front_queue.GrabImage);

				MIL_STRING_STREAM QueueStream;
				pushMoveQueue(front_queue.system_id, "C:/output_image/" + s_idx.str() + "_" + d_idx.str() + "_" + to_string(front_queue.timestamp) + ".tiff");
			}
			else {
