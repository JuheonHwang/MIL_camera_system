...
void SaveGrabbedImage(MIL_ID GrabbedImage, MIL_STRING file_name);
void MFTYPE SlaveSaveGrabbedImage(MIL_ID Func);
...
void SaveGrabbedImage(MIL_ID GrabbedImage, MIL_STRING file_name)
{
	...
	MfuncAlloc(MIL_TEXT("SaveGrabbedImage"),
		2, SlaveSaveGrabbedImage,
		M_NULL, M_NULL,
		M_USER_FUNCTION + 1,
		M_DEFAULT,
		&Func);

	MfuncParamMilId(Func, 1, GrabbedImage, M_IMAGE, M_IN);
	MfuncParamFilename(Func, 2, file_name.c_str(), file_name.length()+1, M_IN);
	//MfuncParamMilText(Func, 2, file_name, file_name.length(), M_IN);
...
{
	MIL_ID GrabbedImage;
	MIL_TEXT_PTR file_name;

	MfuncParamValue(Func, 1, &GrabbedImage);
	MfuncParamValue(Func, 2, &file_name);

	MbufControl(GrabbedImage, M_LOCK, M_DEFAULT);

	MbufSave(file_name, GrabbedImage);

	MbufControl(GrabbedImage, M_UNLOCK, M_DEFAULT);
}
...
int MosMain() {
	...
	MIL_DOUBLE AcquisitionFrameRate = 60;
	...
	*/
	...
	MdigControlFeature(MilDigitizer[1], M_FEATURE_VALUE, MIL_TEXT("ExposureMode"), M_TYPE_STRING, MIL_TEXT("Timed"));
	MdigControlFeature(MilDigitizer[2], M_FEATURE_VALUE, MIL_TEXT("ExposureMode"), M_TYPE_STRING, MIL_TEXT("Timed"));
	MdigControlFeature(MilDigitizer[3], M_FEATURE_VALUE, MIL_TEXT("ExposureMode"), M_TYPE_STRING, MIL_TEXT("Timed"));
...
MIL_INT MFTYPE ProcessingFunction(MIL_INT HookType, MIL_ID HookId, void* HookDataPtr)
{
	...
	if (UserHookDataPtr->system_location == M_DMIL_REMOTE)
	{
	...
	SaveGrabbedImage(SaveBufferId, file_name);
